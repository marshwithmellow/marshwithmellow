    ...
    >
      ...
      >
        <div class="app-name-box">
          <img :src="item.img" class="app-logo" alt="" v-if="item.name" />
          <div class="app-name" v-if="item.name">{{ item.name }}</div>
          <img
            v-if="item.collect && item.name"
...
const emits = defineEmits(["openOverlay", "skip", "toLogin"]);
// type Option = {
//   value: string;
//   label: string;
// };
